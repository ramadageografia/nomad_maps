...
4. Para integrar com mapa existente (se usar Leaflet):
javascript
// Exemplo de integração com Leaflet
function addFestivalMarkersToMap() {
    // Coordenadas dos festivais (exemplo)
    const festivalLocations = [
        { id: 1, lat: -22.9068, lng: -43.1729, title: "Rock in Rio" },
        { id: 2, lat: 48.1351, lng: 11.5820, title: "Oktoberfest" },
    ...
    festivalLocations.forEach(loc => {
        ...
        const marker = L.marker([loc.lat, loc.lng])
            .addTo(map) // 'map' é sua variável do mapa Leaflet
            .bindPopup(`
                <strong>${loc.title}</strong><br>
                <a href="#package-${loc.id}" style="color: #3498db;">
                    Ver pacote completo →
                </a>
            `);
    });
